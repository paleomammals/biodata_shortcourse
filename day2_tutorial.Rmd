...
yrange <- range(df$lat,na.rm = T) + c(-1,1)
...
#make up some fake long/lat points
backgr  <-  randomPoints(mask = climate, n = max(c(nrow(df),500)), p = presvals,
...
points(transform_lon(backgr[,"x"]),backgr[,"y"],col = "blue") #there they are
```
...
This is a linear model, so you can look at the coefficents to estimate how much influence each variable has on the species distribution. (Remember that you can check what the different bioclimatic variables mean at <https://www.worldclim.org/data/bioclim.html>.)
...
```{r}
plotpredictors <- c("bio5","bio6","bio7")
...
In these box plots, the points with x = 1 show the values of the predictor at the real occurrence points, while points with x = 0 show the values of the predictor variable at each of the fake absence points that we added. For my example species, compard to the absence points, the presence points have higher values of bio5 and bio6, and lower values of bio7. That is, blue morpho butterflies prefer places with warm temperatures in both summer and winter, and with not much variation through the year.
...
One more model algorithm. This one is called "MaxEnt" (for "Maximum Entropy"), and it's a machine learning model. That is, it uses /part/ of the data set of occurrence locations to create its initial estimate of the species' climate niche; then it tests that estimate using the remaining points, and tweaks the model to make it more accurate. This is the algorithm most commonly used to model species distributions in published studies. It doesn't use absence points; instead, it treats all the empty cells on the climate map (all the points without species occurrences) as absence points.  

```{r}
m3 <- maxent(x = climate, p = df[,c("lon","lat")])
```
```{r}
r <- predict(climate, m3, progress = "text")
r <- reload_raster(r)
```

```{r}
plot(r, main = "MaxEnt model prediction with GBIF occurrences",
  xlim = range(df$lon,na.rm = T), 
  ylim = range(df$lat,na.rm = T))
points(df$lon,df$lat,col = rgb(1,0,0,0.5),pch = 20,cex = 0.5)
```
...
**Question:** How well did the MaxEnt model predict your species range compared to the other two, just based on visual comparison?
...
```{r}
set.seed(0) #initialize random number generator
backgr  <-  randomPoints(mask = modern, n = nrow(presvals)*(1/0.85), p = presvals)
...
predlgm <- reload_raster(predlgm)

predmidH <- predict(midH,mod,progress = "text")
predmidH <- reload_raster(predmidH)

prednow <- predict(modern,mod,progress = "text")
prednow <- reload_raster(prednow)

pred2070 <- predict(future,mod,progress = "text")
pred2070 <- reload_raster(pred2070)
```

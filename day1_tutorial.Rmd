...
Then we transform all the negative longitude values so that the range goes from 0 to 360 instead of from -180 to 180. This will allow us to plot it on our map.
```{r}
westlongitudes  <-  which(df$lon < 0)
df[westlongitudes,"lon"]  <-  360 + df[westlongitudes,"lon"] 
...
map("world2",col = "lightgreen") #generate the map
map.axes() #label the axes (longitude and latitude) 
points(df$lon,df$lat,col = "red",pch = 20) #plot the species occurrence points
```
...
map("world2",col = "lightgreen",
     ...
     ylim = range(df$lat,na.rm = T))
map.axes()
points(df$lon,df$lat,col = "red",pch = 20) #plot again with only the real data
```
...
```{r}
test1  <-  which(df$lon < 250) #tagging all points that aren't in the Americas, by longitude
map("world2",col = "lightgreen",
     xlim = range(df$lon,na.rm = T), 
     ylim = range(df$lat,na.rm = T))
map.axes()
...
points(df$lon,df$lat,col = "red",pch = 20)
```
Those all look like reasonable places. In a longer-term research project intended for publication, you would spend a lot more time on the data cleaning step, and indeed there are programs and functions for doing exactly that, but for today let's leave it here.
...
You can plot any one of the layers to have a look at it. (Note the double brackets "[[]]" in the call to the 'climate' object -- this is required in order to call a component of a /list/ object.)

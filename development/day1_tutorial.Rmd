...
-   The box **without** the two hash characters (\##) contains the **command**, which is [the text that you will run in R]{.underline}. To run something in R, simply copy the text in the gray codebox into your console and press Return.
...
![Screenshot of the RStudio panels](images/R Console Screenshot.png)\
...
Now, let's try a quick plot of the data.
```{r}
...
Overlay your species occurrences with each of the different bioclimatic data layers in `bioclim` (`bio1` through `bio19`).

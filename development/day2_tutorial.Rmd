...
To use them we're going to need to make some points in places where your species *isn't* present (or at least has not been documented to occur), so that we can compare them to the places where the species was actually found. The other model just used the background.
...
The blue circles are the fake long/lat points that we just made up for locations where the species is not documented to occur. We'll extract these "absent" climate values and assemble them into a table with the "presence" values.

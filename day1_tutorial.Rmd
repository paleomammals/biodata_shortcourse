...
# How to use this tutorial document
...
More details about using logical expressions to subset data can be found [here](https://github.com/hbctraining/Intro-to-R/blob/master/lessons/04_introR-data-wrangling.md#selecting-using-indices-with-logical-operators) if you want it.
...
In this section, the examples will be done using the species *Myrmecophaga tridactyla*, the giant anteater. Wherever you see its name, you'll substitute your own species' name.
...
```{r}
require(dismo)
gbif('Myrmecophaga','tridactyla',geo = FALSE, download = FALSE) #find out the total number of occurrences for this species in the database -- if this doesn't match the number of occurrences you see on the website, you should see if you typed something wrong!
df<-gbif('Myrmecophaga','tridactyla',geo=TRUE) #download all the occurrences with longitude and latitude data, which may not be all of them
```
...
```
We should also make sure we're only dealing with live observations, not with fossil or preserved specimens.

```{r}
notobs<-which(!(df$basisOfRecord=="HUMAN_OBSERVATION" | df$basisOfRecord=="OBSERVATION" | df$basisOfRecord=="OCCURRENCE"))
table(df[notobs,"basisOfRecord"])
plot(wrld_simpl,axes=TRUE,col="lightgreen",
     xlim=range(df$lon,na.rm=T), 
     ylim=range(df$lat,na.rm=T)) #plot again with only the real data
points(df$lon,df$lat,col="red",pch=20)
points(df[notobs,]$lon,df[notobs,]$lat,col="black",pch=21)
```

The points that aren't from actual observations are circled in black. We'll remove these points as well:
...
Plot it again to see if anything looks like it's in the wrong place.
...
My example species, the giant anteater *Myrmecophaga tridactyla*, lives in South and Central American forests. The point on the US/Mexico border is probably an error, and the one in Yucatan is also suspicious. I'll take a look at them:

```{r}
test1<-which(df$lat>15) #the points in Mexico
plot(wrld_simpl,axes=TRUE,col="lightgreen",
     xlim=range(df$lon,na.rm=T), 
     ylim=range(df$lat,na.rm=T)) #plot again with only the real data
points(df$lon,df$lat,col="red",pch=20)
points(df[test1,]$lon,df[test1,]$lat,col="black",pch=21) #circle the flagged points in black
```
...
df[test1,]
```
Neither of these points is in the GBIF map interface. I suspect they're no good.
...
sfdata<-st_as_sf(df,coords=c("lon","lat")) #this reformats the coordinate points into a special data structure

conc<-concaveman(sfdata,concavity=3) #this is called a concave hull, it's a polygon that contains all the points

conv<-convHull(df[,c("lon","lat")]) #this is called a convex hull, it's just a polygon drawn around all the points that stick out the most
```
...
This isn't very satisfactory as a map of species range, as it doesn't take any notice of whether your species could actually live in any of those places. In the next part we'll look at some environmental data to see if we can figure out a better way.
...
This layer, `bio1`, is the average annual temperature. To see what each of the 19 bioclimatic variables means, look at <https://www.worldclim.org/data/bioclim.html>. Temperature measurements are given in tenths of a degree Celsius; precipitation is in millimeters. Then you can plot your own species occurrence data on top of it, restricting the range the same way we did in part 3:
